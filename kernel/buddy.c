...
void bit_buddy(char *array, int index){
...
bd_malloc(uint64 nbytes)
{
  ...
  bit_buddy(bd_sizes[k].alloc,blk_index(k,p)/2);
  for(; k > fk; k--) {
    ...
    bit_buddy(bd_sizes[k-1].alloc, blk_index(k-1, p)/2);
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_buddy(bd_sizes[k].alloc, bi/2);
...
bd_initfree_pair(int k, int bi, int left) {
  ...
  if(bit_isset(bd_sizes[k].alloc,bi/2)){
    int l = (buddy > bi) ? buddy : bi,r = (buddy < bi) ? buddy : bi;
    free = BLK_SIZE(k);
    if(left == 1)
      lst_push(&bd_sizes[k].free, addr(k, l));
    else
      lst_push(&bd_sizes[k].free, addr(k, r));
  }
  return free;
}
